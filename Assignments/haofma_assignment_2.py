...
def import_data(filename):
    ...
    for line in content:
        ...
        for i in line:
            i = i.split(',')
            pokemon_name = i[0]
            pokemon_type = i[1].lower()
            pokemon_hp = float(i[2])
            pokemon_attack = float(i[3])
            pokemon = [pokemon_name, pokemon_type, pokemon_hp, pokemon_attack]
...
def tournament(participants):
    ...
    # 3. Return WINS list
    ######################
    wins = len(participants) * [0]
    
    index = 0
    for i in participants:
        index_rival = index + 1
        for rival in participants[index: ]:
            if index_rival < len(participants):
                home = fight(i, rival, 1)
                if home[0] == 1:
                    wins[index] += 1
                else:
                    wins[index_rival] += 1
                away = fight(i, rival, 2)
                if away[0] == 1:
                    wins[index] += 1
                else:
                    wins[index_rival] += 1
